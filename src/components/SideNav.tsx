...
const NAV_ITEMS: NavItem[] = [
  { to: '/',          end: true, icon: 'map',      labelKey: 'nav.map',      color: '#ff7e5f', glow: 'rgba(255,126,95,0.45)' },
  { to: '/battle',              icon: 'swords',    labelKey: 'nav.battle',   color: '#a78bfa', glow: 'rgba(167,139,250,0.45)' },
  { to: '/practice',            icon: 'exercise',  labelKey: 'nav.practice', color: '#4ade80', glow: 'rgba(74,222,128,0.45)' },
  { to: '/history',             icon: 'history',   labelKey: 'nav.history',  color: '#60a5fa', glow: 'rgba(96,165,250,0.45)' },
];
  ...
  return (
    <aside
      ...
      onMouseEnter={(e) => {
        ...
        (e.currentTarget as HTMLElement).style.boxShadow = '0 0 60px rgba(0,0,0,0.45), 4px 0 32px rgba(255,126,95,0.08)';
      }}
      onMouseLeave={(e) => {
    ...
    >
      ...
      <div className="pointer-events-none absolute inset-0 opacity-30"
        style={{ background: 'radial-gradient(ellipse at 50% 0%, rgba(255,126,95,0.12) 0%, transparent 70%)' }}
      />
      ...
      <div className="mb-6 flex w-full items-center px-4">
        <div className="relative flex items-center justify-center flex-shrink-0 w-10 h-10">
          ...
          <img
            alt="Logo"
            className="relative h-8 w-8 object-contain drop-shadow-[0_0_8px_rgba(255,126,95,0.6)]"
        ...
        <span
          ...
          style={{ opacity: 0, transition: 'opacity 0.2s ease 0.1s', minWidth: 0 }}
          ref={(el) => {
            if (!el) return;
      ...
      {/* Top Divider */}
      <div className="w-full h-px mb-4" style={{ background: 'linear-gradient(90deg, transparent, var(--cg-border), transparent)' }} />
        ...
        {NAV_ITEMS.map((item) => (
          <NavLink
            ...
            style={({ isActive }) => isActive ? {
              background: 'linear-gradient(90deg, rgba(255,126,95,0.15) 0%, transparent 100%)',
      ...
      <div className="w-full px-2">
        <div className="flex items-center gap-3 rounded-xl px-3 py-3 transition-all duration-200 hover:bg-[color:var(--cg-container-a16)] cursor-pointer group">
          ...
          <div className="relative flex-shrink-0 w-8 h-8">
            <svg className="absolute inset-0 animate-spin-slow" width="32" height="32" viewBox="0 0 32 32">
              <circle cx="16" cy="16" r="14" fill="none" stroke="rgba(255,126,95,0.15)" strokeWidth="2" />
              <circle cx="16" cy="16" r="14" fill="none" stroke="#ff7e5f" strokeWidth="2"
          ...
          >
            <div className="text-[12px] font-bold text-[color:var(--cg-text)] whitespace-nowrap">Player</div>
            <div className="text-[10px] text-[color:var(--cg-coral)] font-semibold whitespace-nowrap">Lv.1 Novice</div>
          </div>
        </div>
      </div>
    </aside>
