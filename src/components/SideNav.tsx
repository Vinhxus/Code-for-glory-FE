...
function SideNav() {
  ...
  // ===== Responsive: phát hiện mobile & trạng thái ẩn/hiện sidebar =====
  ...
  useEffect(() => {
    const checkMobile = () => {
      ...
      setIsMobile(mobile);
      // Chuyển sang desktop thì tự đóng menu mobile luôn, xử lý ngay trong
      // cùng sự kiện resize thay vì 1 effect riêng phản ứng theo isMobile
  ...
  return (
    <>
      {/* ===== Nút Hamburger chỉ hiện trên mobile để ẩn/hiện sidebar ===== */}
      {isMobile && (
        <button
          type="button"
          onClick={() => setMobileOpen((prev) => !prev)}
          aria-label={mobileOpen ? 'Đóng menu' : 'Mở menu'}
          className="fixed left-0 top-16 z-[70] flex h-8 w-8 items-center justify-center rounded-xl border border-[color:var(--cg-border)] bg-[color:var(--cg-sidebar)]"
          style={{ boxShadow: '0 0 20px rgba(0,0,0,0.35)' }}
        >
          <span className="material-symbols-outlined text-[20px] text-[color:var(--cg-text)]">
            {mobileOpen ? 'close' : 'menu'}
          </span>
        </button>
      )}

      {/* ===== Lớp backdrop, tap ra ngoài để đóng sidebar mobile ===== */}
      {isMobile && mobileOpen && (
        <div
      ...
      <aside
        ...
        style={{
          ...
          transform: isMobile
            ...
            : 'translateX(0)',
          transition:
            'width 0.28s cubic-bezier(0.4,0,0.2,1), box-shadow 0.28s ease, transform 0.28s cubic-bezier(0.4,0,0.2,1)',
          overflow: 'hidden',
