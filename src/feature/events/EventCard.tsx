...
const COLOR_MAP: Record<string, { badge: string; text: string; accent: string }> = {
  coral: { badge: 'text-[#ff7e5f] bg-[#ff7e5f1a] border border-[#ff7e5f40]', text: 'text-[#ff7e5f]', accent: '#ff7e5f' },
  purple: { badge: 'text-[#a78bfa] bg-[#a78bfa1a] border border-[#a78bfa40]', text: 'text-[#a78bfa]', accent: '#a78bfa' },
  amber: { badge: 'text-[#fbbf24] bg-[#fbbf241a] border border-[#fbbf2440]', text: 'text-[#fbbf24]', accent: '#fbbf24' },
};
...
export default function EventCard({ event, onViewDetails, animationDelay = 0 }: Props) {
  ...
  return (
    <div
      ...
      style={{ borderLeftColor: colors.accent, animationDelay: `${animationDelay}ms` }}
    >
      {/* Left */}
      ...
      <div className="flex flex-col items-start md:items-end gap-2.5 w-full md:w-auto flex-shrink-0">
        <div className={`flex items-center gap-1.5 text-xs font-bold ${isOngoing ? 'text-[#4ade80]' : 'text-[#60a5fa]'}`}>
          <div className={`h-1.5 w-1.5 rounded-full ${isOngoing ? 'bg-[#4ade80] animate-status-pulse' : 'bg-[#60a5fa]'}`} />
          {event.status}
        </div>
