...
function Practice() {
  ...
  const surveyData = useMemo(() => {
    try {
      ...
      console.error('Failed to parse survey data from localStorage:', error);
    }
    return null;
  }, []);
          ...
          <section className="grid grid-cols-1 gap-5 xl:grid-cols-[400px_1fr_340px]">
            ...
            <div className="rounded-2xl border border-[color:var(--cg-border)] bg-[color:var(--cg-container-a16)] backdrop-blur-md overflow-hidden flex flex-col animate-slide-left">
                  ...
                  {(['documentation', 'video'] as DocTab[]).map((t2) => (
                    ...
                    >
                      <span className="material-symbols-outlined text-[14px]">
                        {t2 === 'documentation' ? 'description' : 'play_circle'}
                      </span>
              ...
              <div className="p-5 space-y-5 flex-1 overflow-y-auto">
                {/* Video player mockup */}
                <div
                  className="relative overflow-hidden rounded-xl border border-[color:var(--cg-border)] bg-[color:var(--cg-bg-a55)]"
                  style={{ minHeight: 180 }}
                >
                  <div
                    className="absolute inset-0"
                    style={{
                      background:
                        'radial-gradient(circle at 20% 20%,rgba(0,255,255,0.08),transparent 55%),radial-gradient(circle at 80% 70%,rgba(255,126,95,0.1),transparent 55%)',
                    }}
                  />
                  <div className="relative h-[180px] flex items-center justify-center">
                    <button
                      type="button"
                      className="h-14 w-14 rounded-full neon-btn flex items-center justify-center"
                      title="Play"
                    >
                      <span className="material-symbols-outlined text-[28px] text-[#0f0b3c] pl-0.5">
                        play_arrow
                      </span>
                    </button>
                ...
                {tab === 'documentation' ? (
                  <>
                    <div>
                      ...
                      <p className="mt-2 text-sm leading-relaxed text-[color:var(--cg-text-muted)]">
                        Async functions improve the syntax of Promises, letting
                        you write asynchronous code that reads synchronously.
                        Mastering concurrent requests requires understanding the
                        event loop.
                      </p>
                    </div>
                    ...
                    <div className="rounded-xl border border-cyan-400/20 bg-cyan-400/5 p-4">
                      ...
                      <p className="text-xs leading-5 text-[color:var(--cg-text-muted)]">
                        Use{' '}
                        <code className="text-cyan-300 bg-cyan-400/10 px-1 py-0.5 rounded text-[11px]">
                          Promise.all()
                        </code>{' '}
                        when you want to initiate multiple async operations and
                        wait for all of them. If any promise fails, the entire
                        block rejects.
                      </p>
                    </div>

                    <div>
                      <h3 className="font-['Lexend'] text-base font-semibold mb-2">
                        🎯 The Task
                      </h3>
                      <p className="text-sm leading-relaxed text-[color:var(--cg-text-muted)]">
                        ...
                        . Handle potential rejection with a{' '}
                        <span className="text-[#FF7E5F] font-semibold">
                          try/catch
                        </span>{' '}
                        block.
                  ...
                  <div className="rounded-xl border border-[color:var(--cg-border)] bg-[color:var(--cg-container-a16)] p-4 text-sm text-[color:var(--cg-text-muted)]">
                    Watch the lecture above, then switch back to{' '}
                    <span className="text-[#ff7e5f]">Documentation</span> for
                    your task.
                  </div>
                )}
              </div>
            ...
            <div className="rounded-2xl border border-[color:var(--cg-border)] bg-[color:var(--cg-container-a16)] backdrop-blur-md overflow-hidden flex flex-col animate-fade-in">
              ...
              <div className="flex items-center justify-between gap-4 px-6 py-3.5 border-b border-[color:var(--cg-border)] bg-[color:var(--cg-bg-a55)]">
                <div className="flex items-center gap-3 text-xs font-semibold text-[color:var(--cg-text-muted)]">
                  ...
                  <span className="flex items-center gap-1.5 text-[color:var(--cg-text)]">
                    <span className="material-symbols-outlined text-[14px] text-[#fbbf24]">
                      code
                    </span>
                    index.js
                  </span>
                ...
                <div
                  className={cx(
                    'rounded-full px-4 py-1 text-[10px] font-extrabold tracking-widest border',
                    attempts >= maxAttempts
                      ? 'bg-red-900/60 border-red-500/30 text-red-300'
                      : attempts >= 4
                        ? 'bg-orange-900/60 border-orange-500/30 text-orange-300'
                        : 'bg-[color:var(--cg-container-a16)] border-[color:var(--cg-border)] text-[color:var(--cg-text-muted)]'
                  )}
                >
                ...
                <div className="h-full rounded-xl border border-[color:var(--cg-border)] bg-[color:var(--cg-bg-a55)] overflow-auto">
                  <pre className="p-6 font-['JetBrains_Mono'] text-[12.5px] leading-7 text-[color:var(--cg-text-muted)]">
                    <code>
                      ...
                      {'      '}
                      <span className="text-blue-400">cb</span>(
                      <span className="text-green-400">null</span>, {'{ '}user,
                      profile{' }'});{'\n'}
                      {'    '}
                      {'});'}
                      {'\n'}
              ...
              <div className="border-t border-[color:var(--cg-border)] bg-[color:var(--cg-bg-a72)] backdrop-blur px-6 py-3.5">
                ...
                <div className="flex items-center gap-3 mb-3">
                  <span className="text-[10px] font-bold text-[color:var(--cg-text-muted)] tracking-widest">
                    PROGRESS
                  </span>
                  <div className="flex-1 h-1 rounded-full overflow-hidden bg-[color:var(--cg-container-a30)]">
                    <div
                      className="xp-bar-fill delay-300"
                      style={
                        { '--progress-target': '45%' } as React.CSSProperties
                      }
                    />
                  </div>
                  <span className="text-[10px] font-semibold text-[#ff7e5f]">
                    45%
                  </span>
                </div>
                <div className="flex flex-wrap items-center justify-between gap-4">
                  <div className="flex items-center gap-4 text-xs font-semibold text-[color:var(--cg-text-muted)]">
                    <button
                      type="button"
                      className="inline-flex items-center gap-1.5 hover:text-[color:var(--cg-text)] transition"
                    >
                      <span className="material-symbols-outlined text-[16px]">
                        restart_alt
                      </span>
                    ...
                    <button
                      type="button"
                      className="inline-flex items-center gap-1.5 hover:text-[#fbbf24] transition"
                    >
                      <span className="material-symbols-outlined text-[16px]">
                        help
                      </span>
                  ...
                  <div className="flex items-center gap-3">
                    <button
                      type="button"
                      className="rounded-xl border border-[color:var(--cg-border)] bg-[color:var(--cg-container-a16)] px-5 py-2 text-xs font-bold transition hover:bg-[color:var(--cg-container-a22)]"
                    >
                      Run Tests
                    </button>
                    <button
            ...
            <div className="rounded-2xl border border-[color:var(--cg-border)] bg-[color:var(--cg-container-a16)] backdrop-blur-md overflow-hidden flex flex-col animate-slide-right">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-1.5 mt-0.5">
                      ...
                      <span className="text-[10px] font-bold tracking-widest text-[color:var(--cg-text-muted)]">
                        LOCKED
                      </span>
                    </div>
                  </div>
              ...
              <div className="flex-1 p-5 flex flex-col items-center justify-center text-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-[color:var(--cg-text-muted)] mt-1 leading-relaxed">
                    Complete your first practice quest to activate the AI
                    Mentor.
                  </p>
                </div>
                <div className="w-full rounded-xl border border-[#ff7e5f]/20 bg-[#ff7e5f]/5 px-4 py-3 text-xs text-[color:var(--cg-text-muted)]">
                  <span className="text-[#ff7e5f] font-bold">1 quest</span> away
                  from unlocking!
                </div>
              </div>
