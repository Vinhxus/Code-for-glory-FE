...
const RANK_STYLES = [
  { ring: '#FFD700', bg: 'rgba(255,215,0,0.12)', border: 'rgba(255,215,0,0.3)', medal: '🥇' },
  { ring: '#C0C0C0', bg: 'rgba(192,192,192,0.12)', border: 'rgba(192,192,192,0.3)', medal: '🥈' },
  { ring: '#CD7F32', bg: 'rgba(205,127,50,0.12)', border: 'rgba(205,127,50,0.3)', medal: '🥉' },
];
...
function Homepage() {
  ...
  const stats = useMemo(() => [
    { label: t('home.stats.solved'), value: '10', icon: 'target', color: '#ff7e5f', cls: 'stat-card-coral' },
    { label: t('home.stats.participants'), value: '1B+', icon: 'groups', color: '#a78bfa', cls: 'stat-card-purple' },
    { label: t('home.stats.gold'), value: '1,250', icon: 'star', color: '#fbbf24', cls: 'stat-card-amber' },
  ], [t]);
    ...
    <div className="min-h-screen bg-[color:var(--cg-bg)] text-[color:var(--cg-text)] selection:bg-[color:var(--cg-coral-a18)] select-none overflow-x-hidden">
      ...
      <div className="pointer-events-none fixed inset-0 z-0 overflow-hidden">
        <div className="absolute inset-0 opacity-[0.07]"
          style={{ backgroundImage: 'radial-gradient(#a78bfa 1px, transparent 1px)', backgroundSize: '36px 36px' }} />
        ...
        <div className="absolute bottom-[5%] left-[30%] h-[400px] w-[400px] rounded-full" style={{ background: 'rgba(167,139,250,0.08)', filter: 'blur(120px)' }} />
      </div>
      ...
      <div className="relative z-10 md:pl-[96px]">
        ...
        <header className="sticky top-0 z-40 px-8 py-4 border-b border-[color:var(--cg-border)] bg-[color:var(--cg-bg-a72)] backdrop-blur-xl">
          <div className="mx-auto flex max-w-7xl items-center justify-between">
            ...
            <nav className="hidden items-center gap-8 text-sm font-medium text-[color:var(--cg-text-muted)] md:flex">
              {[['#quests', t('home.nav.quests')], ['#leaderboard', t('home.nav.leaderboard')], ['#shop', t('home.nav.shop')]].map(([href, label]) => (
                <a key={href} href={href} className="relative transition hover:text-[color:var(--cg-coral)] group">
                  {label}
        ...
        <main className="max-w-7xl mx-auto px-8 py-12 space-y-10">
          ...
          <section className="grid grid-cols-1 items-center gap-12 md:grid-cols-2">
            <div className="space-y-7 animate-fade-in-up">
              ...
              <h1 className="font-['Lexend'] text-5xl font-bold tracking-tight leading-[1.15] md:text-6xl">
                ...
                <span className="gradient-text">{t('home.hero.titleB')}</span>{' '}
                <span className="text-[color:var(--cg-text)]">{t('home.nav.quests')}</span>
              </h1>
              <p className="max-w-lg text-base leading-relaxed text-[color:var(--cg-text-muted)]">
                {t('home.hero.subtitle')}
              </p>
              <div className="flex flex-wrap items-center gap-4 pt-1">
                <button type="button" className="neon-btn px-7 py-3.5 text-sm font-extrabold" onClick={() => navigate('/survey')}>
                  {t('home.hero.start')} →
                </button>
                <Link to="/learning-path"
                  ...
                  <span className="material-symbols-outlined text-[16px] text-[#a78bfa]">route</span>
                  {t('home.hero.roadmap')}
                </Link>
              </div>
              ...
              <div className="flex items-center gap-3 pt-1">
                <span className="text-[11px] font-bold text-[color:var(--cg-text-muted)] tracking-widest">XP</span>
                <div className="flex-1 h-1.5 rounded-full overflow-hidden bg-[color:var(--cg-container-a30)] max-w-[180px]">
                  <div className="xp-bar-fill delay-500" style={{ '--progress-target': '60%' } as React.CSSProperties} />
                </div>
                <span className="text-[11px] font-semibold text-[#ff7e5f]">240 / 400</span>
              </div>
            </div>

            {/* Terminal mockup */}
            <div className="relative animate-fade-in-up delay-200">
              <div className="absolute -inset-6 rounded-[2.5rem] blur-2xl opacity-60"
                style={{ background: 'radial-gradient(ellipse, rgba(255,126,95,0.18) 0%, rgba(79,70,229,0.14) 60%, transparent 100%)' }} />
              <div className="relative overflow-hidden rounded-2xl border border-[color:var(--cg-border)] bg-[color:var(--cg-container-a16)] backdrop-blur-xl shadow-[0_32px_80px_rgba(0,0,0,0.4)]">
                ...
                <div className="flex items-center gap-2 border-b border-[color:var(--cg-border)] bg-[color:var(--cg-bg-a55)] px-5 py-3">
                  ...
                  <span className="ml-3 font-['JetBrains_Mono'] text-xs text-[color:var(--cg-text-muted)]">start_quest.js</span>
                  <div className="ml-auto flex items-center gap-1.5">
                    <div className="h-1.5 w-1.5 rounded-full bg-[#4ade80] animate-status-pulse" />
                    <span className="text-[10px] text-[#4ade80] font-semibold">RUNNING</span>
                  </div>
                </div>
                <pre className="overflow-x-auto p-6 font-['JetBrains_Mono'] text-[13px] leading-7 bg-[color:var(--cg-bg-a55)]">
                  <code>
                    ...
                    {'  '}
                    <span className="text-slate-500">// Load mission assets</span>{'\n'}
                    {'  '}session.<span className="text-blue-400">equip</span>({'{\n'}
                    {'    '}engine: <span className="text-green-400">'V8'</span>,{'\n'}
                    {'    '}compiler: <span className="text-green-400">'Glory_v1'</span>{'\n'}
                    {'  '}{'});'}{'\n\n'}
                    {'  '}
              ...
              <div key={s.label}
                ...
                <div className="flex items-start justify-between">
                  <div className="space-y-2">
                    <div className="text-[11px] font-bold tracking-widest text-[color:var(--cg-text-muted)] uppercase">{s.label}</div>
                    <div className="font-['Lexend'] text-4xl font-extrabold tracking-tight animate-count-up"
                      style={{ animationDelay: `${i * 120 + 200}ms`, color: s.color }}>{s.value}</div>
                  </div>
          ...
          {/* Roadmap Preview */}
          <Link to="/learning-path" id="roadmap"
            ...
            <div className="relative z-10">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <h2 className="font-['Lexend'] text-2xl font-semibold">{t('home.path.title')}</h2>
                  <p className="mt-1 max-w-xl text-sm text-[color:var(--cg-text-muted)]">{t('home.path.subtitle')}</p>
                </div>
                <div className="shrink-0 flex items-center gap-2 rounded-xl border border-[#FF7E5F]/40 bg-[#FF7E5F]/10 px-4 py-2 text-xs font-bold text-[#FF7E5F] transition-all group-hover:bg-[#FF7E5F]/20">
                  Frontend & Backend
                  <span className="material-symbols-outlined text-[16px] transition-transform group-hover:translate-x-1">arrow_outward</span>
                </div>
              </div>
              ...
              <div className="mt-12 mb-4 relative flex flex-col justify-between items-center gap-12 md:flex-row md:px-8">
                <div className="absolute top-1/2 left-0 right-0 h-px hidden md:block"
                  style={{ background: 'repeating-linear-gradient(90deg,rgba(255,126,95,0.3) 0,rgba(255,126,95,0.3) 6px,transparent 6px,transparent 14px)' }} />
                {[
                  { id: 'JS', label: 'Foundations', active: true, color: '#ff7e5f' },
                  { id: '⚙️', label: 'Advanced', active: false, color: '#a78bfa' },
                  { id: '👑', label: 'Mastery', active: false, color: '#fbbf24' },
                ].map((node, i) => (
                  <div key={node.id} className="relative flex flex-col items-center z-10 animate-bounce-in" style={{ animationDelay: `${i * 150}ms` }}>
                    <div className={`w-20 h-20 rounded-[20px] flex items-center justify-center border-2 transition-all duration-300 bg-[color:var(--cg-bg)] ${node.active ? 'scale-110' : 'group-hover:scale-105'}`}
                      style={node.active ? { borderColor: node.color, boxShadow: `0 0 30px ${node.color}40` } : { borderColor: 'var(--cg-border)' }}>
                      <span className="font-['Lexend'] text-xl font-bold" style={{ color: node.active ? node.color : 'var(--cg-text-muted)' }}>{node.id}</span>
                    </div>
                    <div className="mt-3 flex flex-col items-center gap-1">
                      <span className="text-[11px] font-bold text-[color:var(--cg-text-muted)]">{node.label}</span>
                      <div className="h-1 w-8 rounded-full" style={{ background: node.active ? '#4ade80' : 'rgba(100,100,120,0.3)' }} />
                    </div>
                  </div>
                ))}
              </div>
          ...
          <section className="grid grid-cols-1 gap-6 md:grid-cols-3 animate-fade-in-up delay-300">
            ...
            <div id="quests"
              ...
              <div className="absolute inset-0 opacity-40"
                style={{ background: 'radial-gradient(ellipse at 0% 0%, rgba(255,126,95,0.18) 0%, transparent 60%), radial-gradient(ellipse at 100% 100%, rgba(167,139,250,0.12) 0%, transparent 60%)' }} />
              <div className="relative z-10 space-y-4">
                <div className="badge-coral w-fit">
                  <span className="material-symbols-outlined text-[14px]">whatshot</span>
                  {t('home.arena.tag')}
                </div>
                <h2 className="font-['Lexend'] text-3xl font-bold">{t('home.arena.title')}</h2>
                <p className="max-w-xl text-sm leading-relaxed text-[color:var(--cg-text-muted)]">{t('home.arena.subtitle')}</p>
              </div>
              <div className="relative z-10 pt-8">
                <button type="button" className="neon-btn px-6 py-3 text-sm" onClick={() => navigate('/survey')}>
                  {t('home.arena.cta')}
                </button>
              </div>
            </div>
            ...
            {/* Leaderboard */}
            <div id="leaderboard" className="rounded-2xl border border-[color:var(--cg-border)] bg-[color:var(--cg-container-a16)] backdrop-blur-md flex flex-col overflow-hidden card-hover">
              <div className="border-b border-[color:var(--cg-border)] px-6 py-4 bg-[color:var(--cg-bg-a55)] flex items-center justify-between">
                <h3 className="font-['Lexend'] text-base font-semibold">{t('home.top.title')}</h3>
                ...
                {topUsers.map((user, i) => {
                  ...
                  return (
                    <div key={user.rank}
                      ...
                      <div className="flex h-8 w-8 items-center justify-center rounded-lg text-sm font-extrabold flex-shrink-0"
                        style={{ background: rs.bg, border: `1px solid ${rs.border}`, color: rs.ring }}>
                        {rs.medal}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="text-xs font-bold text-[color:var(--cg-text)] truncate">{user.name}</div>
                        <div className="text-[10px] text-[color:var(--cg-text-muted)] font-medium tracking-wide">{user.class}</div>
                      </div>
                      <div className="text-right">
                        <div className="text-xs font-extrabold" style={{ color: rs.ring }}>{user.xp}k</div>
                        <div className="text-[9px] text-[color:var(--cg-text-muted)] font-bold">XP</div>
                      </div>
                    </div>
                  );
                })}
          ...
          {/* Footer */}
          <footer id="shop" className="border-t border-[color:var(--cg-border)] pt-12 text-[color:var(--cg-text-muted)]">
            <div className="grid grid-cols-1 gap-10 md:grid-cols-4">
              <div className="md:col-span-2 space-y-4">
                <div className="flex items-center gap-3">
                  <img src="/component_2_2x.png" alt="Logo" className="h-7 w-7 object-contain" />
                  <span className="font-['Lexend'] text-base font-bold text-[color:var(--cg-text)]">
                    <span className="text-[#ff7e5f]">Code</span>ForGlory
                  </span>
                </div>
                <p className="max-w-sm text-sm leading-relaxed">Gamifying the future of software engineering. Orchestrate your digital universe through mastery and conquest.</p>
                <div className="flex gap-3 pt-2">
                  {['language', 'chat', 'flutter'].map((icon) => (
                    <button key={icon} className="h-9 w-9 flex items-center justify-center rounded-xl border border-[color:var(--cg-border)] bg-[color:var(--cg-container-a16)] transition hover:border-[#ff7e5f]/40 hover:text-[#ff7e5f]">
                      <span className="material-symbols-outlined text-[18px]">{icon}</span>
                    </button>
                  ))}
                </div>
              </div>
              {[[t('home.footer.platform'), ['Courses', 'Arena', 'Pricing']], [t('home.footer.community'), ['Discord', 'Events', 'Guilds']]].map(([title, items]) => (
                <div key={title as string}>
                  <div className="text-xs font-bold tracking-widest text-[color:var(--cg-text)] uppercase mb-4">{title}</div>
                  <div className="space-y-3 text-sm">
                    {(items as string[]).map((item) => (
                      <button key={item} type="button"
                        className="block text-left hover:text-[#ff7e5f] transition-colors"
                        onClick={() => navigate('/survey')}>{item}</button>
                    ))}
                  </div>
                </div>
              ))}
            ...
            <div className="mt-12 flex flex-col items-start justify-between gap-4 border-t border-[color:var(--cg-border)] py-6 text-xs md:flex-row md:items-center">
              <div>© {new Date().getFullYear()} CodeForGlory. All rights reserved.</div>
              <div className="flex items-center gap-6">
                {[t('home.footer.terms'), t('home.footer.privacy'), t('home.footer.support')].map((item) => (
                  <button key={item} type="button" className="hover:text-[color:var(--cg-text)] transition" onClick={() => navigate('/survey')}>{item}</button>
                ))}
              </div>
            </div>
          </footer>
